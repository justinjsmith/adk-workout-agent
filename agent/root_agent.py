...
from agent.analyzer_agent import analyzer_agent
...
3. After the parser returns the structured workout, send the parsed workout JSON to the \
workout_analyzer agent for physiological analysis (energy systems, coaching intent, effort profile).
4. If the email does not contain a workout, respond with a brief summary of what the email contains.

When given a direct command (e.g., "parse this workout text"), route it directly to the \
workout_parser agent without triage, then to the workout_analyzer.
""",
    sub_agents=[triage_agent, parser_agent, analyzer_agent],
)
